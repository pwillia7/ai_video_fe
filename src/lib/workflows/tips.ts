...
const PATCHES: Record<string, TipSection> = {
  ...
  style: {
    heading: "A content LoRA is on",
    items: [
      "One more LoRA is stacked on the diffusion model, behind the turbo one, and the dropdown under the switch says which. The list is curated by hand: a LoRA for some other model family would splice in without an error and produce nothing but noise, so what is offered here is what has actually been run on these graphs.",
      "A LoRA with a trigger word gets it written into the prompt for you, in front of whatever you typed — that is what it was trained to answer to, and the weights alone do very little without it. It goes in deterministically rather than being asked of the prompt director, so it survives both the rewrite and Send my prompt as written. The run detail shows the exact text that went in.",
      "Tape damage picks how worn the VHS one reads: Light, Medium or Heavy, in the LoRA author's own words. Medium is the default, because Turbo's four-step sampler already softens the grain and the stacked strength is half — starting at Light stacks three things that all pull the same way.",
      "That LoRA wants 4:3 and about a megapixel — it was trained on 480p tape. Very low resolutions are unstable, so if a take comes out mushy rather than grainy, check the size before the strength.",
      "It also moves the graph onto a different checkpoint, and that is the part that matters. MiniMax-H3 LoRAs are made for fp16/fp8 weights, and these graphs otherwise run a quantised base that stores the same weights in a rotated form. A LoRA loads into one of those without complaining and then produces warped faces and melting limbs — so the switch loads a checkpoint the chosen LoRA supports for as long as it is on, and puts the rotated one back when it is off.",
      "Which means a run with one of these on costs meaningfully more than a plain one, in time and in VRAM. That is the switch, not your settings. The estimate learns it separately, so the first take in this combination paces itself off a guess and every one after it off your own machine.",
      "Strength and Lighter base belong to the LoRA rather than to the switch, so each one in the list remembers its own. Switching LoRAs finds that one's settings rather than inheriting the last one's, whose range may not even contain them.",
      "Lighter base chooses which supported checkpoint to run. Off is the better one; on is a smaller, lighter checkpoint for some quality. Both are non-rotated, so neither side can produce the melting-limbs failure — it is a straight quality-for-memory trade, and the one to reach for if the default will not fit on your card or is not downloaded.",
      "The take records which LoRA it used, how strong, and on which weights. Two takes with the same switches on can differ by all three and the run's name shows none of them, so the run detail names them and Reuse settings puts them back.",
      "All five video workflows offer this. A LoRA declares a checkpoint per model family, so the same entry works whichever backbone the workflow runs — H3 has two, and each has its own non-rotated build.",
      "On Reference to Video and Remix the VHS pairing is ours rather than the LoRA author's: its model card covers text-to-video and image-to-video only. What is checked is that it applies at all — every one of its modules attaches to the reference checkpoint with no unmatched keys. Whether the look is worth having on a graph whose job is holding a likeness is for you to judge, so compare a take against one without it before trusting it.",
      "Unlike Turbo, SageAttention and Spectrum, this needs no node pack — the loader is a ComfyUI built-in. It does need the LoRA file in `models/loras/` and whichever checkpoint you mean to run. `pnpm check:nodes` reports every LoRA in the list as optional, since you download the ones you use.",
    ],
  },
  sage: {
