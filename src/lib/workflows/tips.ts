    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          "Leave the box empty for an instrumental. The director is told so, and writes the caption around a lead instrument instead of a singer — then, unless you turn Plan the sections off, a second pass writes a wordless section plan into the lyrics field so the piece has a length to play to.",
        ],
      },
      {
        ...
        items: [
          ...
          "Instrumentals come back shortest, because the lyrics field — which is the model's structural channel, not only its words — is empty. Plan the sections fills it instead, with a wordless plan built from the caption: one tag per section, each carrying its own length. [Intro - 16 seconds, solo piano states the theme], and so on.",
          "That gives an instrumental something to play to rather than a mood to stop whenever. Turn it off under the Lyrics box to send nothing and let the model decide, which is what it did before.",
