...
const PATCHES: Record<string, TipSection> = {
  ...
  style: {
    ...
    items: [
      ...
      "It also moves the graph onto a different checkpoint, and that is the part that matters. MiniMax-H3 LoRAs are made for fp16/fp8 weights, and these graphs otherwise run a quantised base that stores the same weights in a rotated form. A LoRA loads into one of those without complaining and then produces warped faces and melting limbs — so the switch loads a supported checkpoint for as long as it is on, and puts the rotated one back when it is off.",
      "Lighter base, under the switch, chooses which of the two supported checkpoints that is. Off is bf16, the better one; on is the scaled fp8, which is much smaller and much easier on VRAM for some quality. Both are non-rotated, so neither side can produce the melting-limbs failure — it is a straight quality-for-memory trade, and the one to reach for if bf16 will not fit on your card or is not downloaded. Like the strength, it is remembered once for the whole app, because it is your machine it is about.",
      ...
      "Unlike Turbo, SageAttention and Spectrum, this needs no node pack — the loader is a ComfyUI built-in. It does need the LoRA itself in `models/loras/`, and whichever base you mean to run: `pnpm check:nodes` reports the default one as required and the lighter one as optional, since you only download the side you use.",
    ],
  },
  sage: {
